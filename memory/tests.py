...
class MemoryTest(unittest.TestCase):
    def setUp(self):
        self.memory = InPlaceMemory(lambda a,b,c,d: True, lambda x: len(x["content"]))
    ...
    def test_limit_idx(self):
        self.memory = InPlaceMemory(lambda idx,ts,size,msg: True if idx < 2 else False, lambda x: len(x["content"]))
        self.memory.append("user1", {"role": "user", "content": "Test message 1"})
    ...
    def test_limit_msg(self):
        self.memory = InPlaceMemory(lambda idx,ts,size,msg: True if msg["role"] == "user" else False, lambda x: len(x["content"]))
        self.memory.append("user1", {"role": "user", "content": "Test message 1"})
    ...
    def test_limit_size(self):
        self.memory = InPlaceMemory(lambda idx,ts,size,msg: True if size < 2 * 15 else False, lambda x: len(x["content"]))
        self.memory.append("user1", {"role": "user", "content": "Test message 1"})
        self.memory.append("user1", {"role": "user", "content": "Test message 2"})
        self.memory.append("user2", {"role": "user", "content": "Test message _"})
        self.assertEqual(len(self.memory.get("user1")), 2)
